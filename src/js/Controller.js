...
function init() {
    ...
    Logic.getFavColors(); // get them from LS and push to state
    runEventListeners();
}
init();
...
function showFavoriteColors() {
    ...
    // rendering:
    savedColors.forEach((colHex, i) => Visual.renderColorElement(undefined, colHex, savedColorsRGBs[i], parentEl, "fullHeart"));
...
function markFavorite(hexString, resultEl) {
    Visual.changeHeartIcon(resultEl);
    ...
    if (isMarkedFavorite) {
        ...
        if (!Visual.favoriteSection.classList.contains("hidden")) {
            ...
            Visual.renderFavorites(savedColors, savedColorsRGBs);
        }
    }
}
